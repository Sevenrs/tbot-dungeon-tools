...
import os
import json
import chardet  # Importa la librería chardet para detección de codificación
...
def unpack_dungeon_bin(input_file, output_folder):
    try:
        ...
        for i, script_name in enumerate(scripts):
            ...
            print(f"Datos decodificados (xor aplicados) para el script {script_name}: {script_decoded[:20]}...")

            # Detectar la codificación usando chardet
            detection_result = chardet.detect(script_decoded)
            ...
            try:
                ...
                decoded_data = script_decoded.decode(encoding, errors='replace').splitlines()
                print(f"Datos decodificados para el script '{script_name}' (primeras líneas): {decoded_data[:5]}")
            except Exception as e:
                print(f"Error al decodificar el script '{script_name}' con la codificación '{encoding}': {e}")
                continue
            ...
            try:
                with open(output_path, 'w', encoding='utf-8') as output_file:
                    json.dump(decoded_data, output_file, ensure_ascii=False, indent=4)
            ...
            try:
                with open(bytearray_output_path, 'w', encoding='utf-8') as bytearray_file:
                    bytearray_file.write("\n".join(decoded_data))  # Guardar como lista de strings
